...
import br.com.guilhermetech.chesssystem.chess.pieces.*;
...
public class ChessMatch {
    ...
    private Piece makeMove(Position source, Position target){
        var p = (ChessPiece)board.removePiece(source);
        p.increaseMoveCount();
        ...
        if (p instanceof King && target.getColumn() == source.getColumn() + 2){
            var sourceTower = new Position(source.getRow(), source.getColumn() + 3);
            var targetTower = new Position(source.getRow(), source.getColumn() + 1);
            var rook = (ChessPiece)board.removePiece(sourceTower);
        ...
        if (p instanceof King && target.getColumn() == source.getColumn() - 2){
            var sourceTower = new Position(source.getRow(), source.getColumn() - 4);
            var targetTower = new Position(source.getRow(), source.getColumn() - 1);
            var rook = (ChessPiece)board.removePiece(sourceTower);
            board.placePiece(rook, targetTower);
            rook.increaseMoveCount();
    ...
    private void undoMove(Position source, Position target, Piece capturedPiece){
        var p = (ChessPiece)board.removePiece(target);
        p.decreaseMoveCount();
        ...
        if (p instanceof King && target.getColumn() == source.getColumn() + 2){
            var sourceTower = new Position(source.getRow(), source.getColumn() + 3);
            var targetTower = new Position(source.getRow(), source.getColumn() + 1);
            var rook = (ChessPiece)board.removePiece(targetTower);
    ...
    private boolean testCheck(Color color){
        ...
        List<Piece> opponentPieces = piecesOnTheBoard.stream()
                .filter(x -> ((ChessPiece)x).getColor().equals(opponent(color)))
